    ...
    <div className="relative min-h-screen bg-[#feeaf0] py-16 px-6 overflow-hidden">
      ...
      {activeItem && (
        <div className="fixed inset-0 z-[999] bg-black/60 flex items-center justify-center px-4">
          <div className="relative w-full max-w-3xl h-[90vh] bg-[#fff5f8] rounded-[2.5rem] shadow-2xl overflow-hidden">

            {/* CLOSE */}
            <button
              onClick={() => setActiveItem(null)}
              className="absolute top-5 right-5 z-20 text-3xl text-[#9a7c85] hover:text-[#d88a9e]"
            >
              ✕
            </button>
            ...
            <div className="h-full overflow-y-auto p-8">
              ...
              <p className="font-elegant text-lg text-[#5c4a50] text-center leading-relaxed mb-8">
                {activeItem.description}
              </p>

              {/* DUMMY CONTENT ĐỂ TEST SCROLL */}
              {Array.from({ length: 12 }).map((_, i) => (
                <p key={i} className="text-[#5c4a50] mb-4">
                  Slime is not just a product — it is joy, texture, curiosity, and calm combined.
                </p>
              ))}
            </div>
          </div>
